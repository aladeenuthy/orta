...
class ProfileOnboardingCubit extends Cubit<ProfileOnboardingState> {
  ProfileOnboardingCubit({required ProfileService profileService})
  ...
  Future<void> savePersonalInformation() async {
    ...
    final Either<AppError, Profile> result = await _profileService
    ...
    result.fold(
      ...
      (Profile profile) => emit(state.toPersonalSaved(profile)),
    );
  }
  ...
  Future<void> saveSkills() async {
    if (state.isLoading) return;
    emit(state.toLoading());
    final Either<AppError, Profile> result = await _profileService.saveSkills(
      skills: state.skills,
    );

    result.fold(
      (AppError error) => emit(state.toError(error.message)),
      (Profile profile) => emit(state.toSkillsSaved(profile)),
    );
  }
}
