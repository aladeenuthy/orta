...
class WorkerProfileRemoteDataSource extends BaseAppRepository {
  WorkerProfileRemoteDataSource({required super.api});

  Future<Either<AppError, WorkerProfile>> getProfile() {
    return makeRequest(() async {
      final response = await get(Endpoints.workerProfile);
      final Map<String, dynamic> body = Map<String, dynamic>.from(
        response.data as Map,
      );

      return right<AppError, WorkerProfile>(
        WorkerProfile.fromJson(Map<String, dynamic>.from(body['data'] as Map)),
      );
    });
  }
  ...
  Future<Either<AppError, WorkerProfile>> updateProfile({
    ...
    return makeRequest(() async {
      ...
      return right<AppError, WorkerProfile>(
        WorkerProfile.fromJson(Map<String, dynamic>.from(data as Map)),
      );
    });
  }
  ...
  Future<Either<AppError, List<AvailabilityDay>>> getAvailability() {
    return makeRequest(() async {
      final response = await get(Endpoints.workerAvailability);
      final Map<String, dynamic> body = Map<String, dynamic>.from(
        response.data as Map,
      );
  ...
  Future<Either<AppError, List<UnavailabilityPeriod>>> getUnavailability({
    ...
    return makeRequest(() async {
      final response = await get(
        Endpoints.workerUnavailability,
  ...
  Future<Either<AppError, Unit>> deleteUnavailability(String id) {
    return makeRequest(() async {
      await delete(Endpoints.deleteWorkerUnavailability(id));
